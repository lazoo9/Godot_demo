...
@export var generate_collection_time_gap: Vector2 = Vector2(1, 3)
...
func genarate_collection() -> void:
	...
	await get_tree().create_timer(randf_range(generate_collection_time_gap.x, generate_collection_time_gap.y)).timeout
	genarate_collection()
